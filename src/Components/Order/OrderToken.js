...
export const OrderToken = () => {
  ...
  useEffect(() => {
    const fetchChannelingDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8080/FreshSip/adminUser/${userID}`);
        if (!response.ok) {

          throw new Error('Failed to fetch Channeling data');
        }
        const fetchedChanneling = await response.json();
        setUser(fetchedChanneling.user);
      } catch (error) {
            ...
            <form ref={form} onSubmit={sendEmail}>
              ...
              <div className='form-group row mb-2' >
                ...
                <div className='col-sm-10'>
                  <input className='form-control' type="text" name="message1" value={user?.firstName || ''}  readOnly />
                </div>
              </div>
              <div className='form-group row mb-2' >
