...
function OrderAdmin() {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8080/FreshSip/Order/admin/OrderItemsByOrdersItemsDates/${date}`);
        if (!response.ok) {
  ...
  const handleSubmitClick = async (orderid) => {
    ...
    const fetchOrderDetails = async (orderid) => {
      console.log(orderid);
      try {
        const response = await fetch(`http://localhost:8080/FreshSip/Order/adminUser/CartById/${orderid}`);
        if (!response.ok) {
          throw new Error('Failed to fetch order data');
        }
        const orderData = await response.json();
    ...
    if (orderDetails) {
      ...
      if (orderid) {
        try {
          ...
          const orderResponse = await fetch(`http://localhost:8080/FreshSip/Order/adminUser/CartById/${orderid}`);
